...
function Point(x, y, z, size, color) {
    ...
    this.draw = function (bubbleShape, dx, dy) {
       ...
       if (bubbleShape == "heart") {
            ...
            ctx.beginPath();
            ctx.arc(this.curPos.x*ratio + dx + window.innerWidth / 5 , this.curPos.y*ratio + dy + 25, this.radius, 0, Math.PI * 2, true);
            ctx.fill();
        }
    };
}
